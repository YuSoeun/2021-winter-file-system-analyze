...
namespace util::parser {
  ...
  bool PListParser::init_with_path(string const& path)
  {
    ...
    #if defined(USE_BPLIST)
    util::parser::PropertyList bplist;
    if (bplist.open(path))
    {
  ...
  auto PListParser::get_dict(char const* key, char const* where) const -> map<string, string>
  {
    return enumerate(key, where);
  }

  auto PListParser::get_dict(char const* key) const -> map<string, string>
  {
  ...
  auto PListParser::enumerate(string const& key, char const* where) const -> map<string, string>
  {
